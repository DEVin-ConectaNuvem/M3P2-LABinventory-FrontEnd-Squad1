    ...
    <div class="container">
        ...
        <div class="accordion" v-for="collaborator in collaborators" :key="collaborator.id">
            <div class="accordion-item">
                ...
                <div :id="'collapse' + collaborator.id" class="accordion-collapse collapse"
                    ...
                    <div class="accordion-body">
                        ...
                        <br>
                        <button class="btn btn-warning mt-2" @click="editCollab(collaborator.id)"><i
        ...
        <p v-show="collaborators.length === 0 && inputSearch"> Não há colaboradores cadastrados com este nome</p>
    </div>
</template>
...
const totalPages = computed(() => {
    if (inputSearch.value) {
        return Math.ceil(store.state.collaboratorModule.collaborators.filter((collaborator) =>
            collaborator.name.toLowerCase().includes(inputSearch.value.toLowerCase())
        ).length / perPage.value);
    } else {
    return Math.ceil(store.state.collaboratorModule.collaborators.length / perPage.value);
    }
})

const collaborators = computed(() => {
    if (inputSearch.value) {
        let total = store.state.collaboratorModule.collaborators.filter((collaborator) =>
            collaborator.name.toLowerCase().includes(inputSearch.value.toLowerCase())
        );
        total = total.slice(
