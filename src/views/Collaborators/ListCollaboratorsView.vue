<template>
  <div class="container mt-3">
    ...
    <div
      ...
      v-for="collaborator in allCollabsPaginate"
      :key="collaborator.id"
    >
      <div class="accordion-item">
        <h2 class="accordion-header" :id="collaborator.id">
          ...
          >
            <img
              v-if="collaborator.imageUser"
              :src="collaborator.imageUser"
            ...
            />
            <p
              v-text="collaborator.name"
              class="ms-2 nameCollab"
              :data-testid="`colab-list-name-${collaborator.id}`"
            ></p>
            <p
              class="ms-2 text-center"
              v-text="'#' + collaborator.position"
            ></p>
          </button>
        </h2>
        <div
          ...
          <div class="accordion-body">
            <div class="row">
              <div class="col-sm-12 col-md-6">
                <strong>Email: </strong>
                <span :data-testid="`colab-list-email-${collaborator.id}`">
                  {{ collaborator.email }}
                </span>
                <br />
                <strong>Telefone: </strong
                ><span :data-testid="`colab-list-phone-${collaborator.id}`">
                  {{ collaborator.phone }}
                </span>
                <br />
                <strong>Cargo: </strong
                ><span :data-testid="`colab-list-position-${collaborator.id}`">
              ...
              <div class="col-sm-12 col-md-6">
                ...
                ><span :data-testid="`colab-list-createdAt-${collaborator.id}`">
                  {{ formatDate(collaborator.createdAt, 'DD/MM/yyyy hh:mm') }}
                </span>
                <br />
                <strong>última modificação: </strong
                ><span data-testid="colab-list-updateAt">
                  {{ formatDate(collaborator.updatedAt, 'DD/MM/yyyy hh:mm') }}
                </span>
                <br />
              </div>
            </div>
    ...
    <p
      class="text-danger"
      v-show="allCollabsCount === 0 && inputConfig.searchText"
      data-testid="colab-list-not-found"
    >
      Não há colaboradores cadastrados com este
    ...
    >
      ...
      <router-link :to="{ name: 'colaboradores' }"
