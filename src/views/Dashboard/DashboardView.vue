  ...
  <div class="container mt-3">
    ...
    <section class="row cards">
      ...
      >
        <template v-slot:icon
          ><i class="widgets-icons-2 bg-gradient-scooter fa-solid fa-users"></i
        ></template>
      ...
      >
        <template v-slot:icon
          ><i
            class="widgets-icons-2 bg-gradient-bloody fa-solid fa-boxes-stacked"
          ></i
        ></template>
      ...
      >
        <template v-slot:icon
          ><i
            class="widgets-icons-2 bg-gradient-ohhappiness fa-solid fa-sack-dollar"
          ></i
        ></template>
        <template v-slot:count
          >R$ {{ tweenedValueTotal.number.toFixed(2) }}</template
        >
      ...
      >
        <template v-slot:icon>
          <i
            class="widgets-icons-2 bg-gradient-blooker fa-solid fa-arrow-right-arrow-left"
          ></i>
        </template>
        <template v-slot:count>{{
          tweenedItemsLoaned.number.toFixed(0)
        }}</template>
    ...
    <section class="row">
      <cards-products
        ...
        @click="itemInfos(item.codPatrimonio)"
      >
    ...
    <m-dialog v-model="show" title="Informações Adicionais">
      <div class="row text-center">
        ...
        <div class="col-sm-12 col-md-6 ml-0">
          ...
          <p>
            ...
            <span
              :class="
                item.collaborator ? 'badge bg-primary' : 'badge bg-success'
              "
            >
              {{ item.collaborator ? item.collaborator : "Disponível" }}</span
            >
          </p>
        </div>
        <div class="mt-1">
      ...
      <template v-slot:footer>
        <button class="btn btn-secondary me-2" @click="toggleModal">
          Cancelar
        </button>
        <button class="btn btn-primary" @click="editItem(item.codPatrimonio)">
...
gsap.to(tweenedCollabs, {
  duration: 1,
  number: Number(collabsCount.value.length) || 0,
});
gsap.to(tweendItems, {
  duration: 1,
  number: Number(itemsCount.value.length) || 0,
});
gsap.to(tweenedValueTotal, {
  duration: 1,
  number: Number(valueTotalItems.value) || 0,
});
gsap.to(tweenedItemsLoaned, {
  duration: 1,
  number: Number(itemsLoaned.value.length) || 0,
});
